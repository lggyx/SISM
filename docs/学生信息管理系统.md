...
> **说明**：所有 Servlet 均通过对应的 DataStore 访问数据库。例如 `IndexServlet`、`AddStudentServlet` 等依赖 `StudentDataStore`；`ScoreListServlet`、`AddScoreServlet` 等依赖 `ScoreDataStore`；`LoginServlet`、`ChangePasswordServlet` 依赖 `AdminDataStore`。
...
![1778636443657](image/学生信息管理系统/1778636443657.png)
...
![1778636463507](image/学生信息管理系统/1778636463507.png)
...
![1778636498019](image/学生信息管理系统/1778636498019.png)

- 表单包含学号、姓名、性别、年龄、班级输入项
- 提交后验证学号唯一性

### 3.3.4 修改学生页面

![1778636514318](image/学生信息管理系统/1778636514318.png)
...
![1778636558355](image/学生信息管理系统/1778636558355.png)
...
![1778636629729](image/学生信息管理系统/1778636629729.png)
...
| -------- | ---------------- | --------------------------- | --------------- | ------------------ | ---------------------------------- | -------- |
| TC-001   | 正确账号密码登录 | MySQL已启动，数据库已初始化 | admin / 123456  | 登录成功，跳转首页 | 登录成功，Session创建，跳转首页    | 通过     |
| TC-002   | 错误密码登录     | 同上                        | admin / wrong   | 登录失败，提示错误 | 页面提示"账号或密码错误"           | 通过     |
| TC-003   | 空用户名         | 同上                        | 空 / 123456     | HTML5阻止提交      | 浏览器提示"请填写此字段"           | 通过     |
| TC-004   | 未登录访问首页   | 未登录                      | 直接访问 /index | 重定向到登录页     | 自动重定向至登录页面               | 通过     |

### 3.4.2 学生管理测试

| 用例编号 | 用例名称     | 前置条件         | 输入数据                    | 预期结果              | 实际结果                                     | 测试状态 |
| -------- | ------------ | ---------------- | --------------------------- | --------------------- | -------------------------------------------- | -------- |
| TC-005   | 正常添加学生 | 已登录           | 202313/钱七/男/20/计算机1班 | 添加成功，首页显示    | 添加成功，首页列表显示该学生                 | 通过     |
| TC-006   | 学号重复     | 已登录           | 202301/...                  | 提示学号已存在        | 页面提示"学号已存在，请更换"                 | 通过     |
| TC-007   | 学号非6位    | 已登录           | 12345/...                   | 提示学号格式错误      | 页面提示"学号必须是6位数字"                  | 通过     |
| TC-008   | 分页展示     | 已登录，12条数据 | 访问 /index                 | 显示第1页5条+分页导航 | 首页显示5条，分页导航显示"共12条，第1/3页"   | 通过     |
| TC-009   | 模糊查询     | 已登录           | 关键字"张"                  | 显示匹配的学生        | 列表显示姓名含"张"的学生记录                 | 通过     |
| TC-010   | 正常删除     | 已登录           | 点击删除并确认              | 学生从列表移除        | 确认后删除成功，首页列表不再显示该学生       | 通过     |

### 3.4.3 成绩管理测试

| 用例编号 | 用例名称   | 前置条件 | 输入数据         | 预期结果      | 实际结果                           | 测试状态 |
| -------- | ---------- | -------- | ---------------- | ------------- | ---------------------------------- | -------- |
| TC-011   | 添加成绩   | 已登录   | 202301/数学/85.5 | 添加成功      | 成绩添加成功，成绩列表显示该记录   | 通过     |
| TC-012   | 成绩超范围 | 已登录   | 202301/数学/105  | 提示0-100之间 | 页面提示"成绩必须在0-100之间"      | 通过     |
| TC-013   | 删除成绩   | 已登录   | 点击删除并确认   | 成绩移除      | 确认后成绩删除成功，列表不再显示   | 通过     |

### 3.4.4 密码修改测试

| 用例编号 | 用例名称         | 前置条件 | 输入数据                             | 预期结果               | 实际结果                                   | 测试状态 |
| -------- | ---------------- | -------- | ------------------------------------ | ---------------------- | ------------------------------------------ | -------- |
| TC-014   | 正常修改密码     | 已登录   | 原密码123456/新密码abc123/确认abc123 | 修改成功，要求重新登录 | 修改成功，提示重新登录，跳转至登录页       | 通过     |
| TC-015   | 原密码错误       | 已登录   | 原密码wrong/...                      | 提示原密码错误         | 页面提示"原密码错误"                       | 通过     |
| TC-016   | 两次新密码不一致 | 已登录   | .../abc123/abc456                    | 提示密码不一致         | 页面提示"两次输入的新密码不一致"           | 通过     |
| TC-017   | 新密码太短       | 已登录   | .../123/123                          | 提示不少于6位          | 页面提示"新密码长度不能少于6位"            | 通过     |
